...
document.addEventListener('click', e => {    
    ...
    if(e.ctrlKey == true) {
        ...
        if(itemIndex > 4) {
            for(let i = 0; i < tableRowsCount; i++) {
                if(lastDateCells[i].innerHTML == 'Down' || cells[i][itemIndex].innerHTML == 'Down') cells[i][3].innerHTML = '-'
                else cells[i][3].innerHTML = ((Number(lastDateCells[i].innerHTML) - Number(cells[i][itemIndex].innerHTML)).toFixed(1))               
